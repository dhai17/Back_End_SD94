...
public interface SanPhamRepository extends JpaRepository<SanPham, Long> {
    ...
    @Query(value = "select * from san_pham \n" +
            ...
            "         where mau_sac_id = ? and san_pham.is_deleted = false and trang_thai = 0", nativeQuery = true)
    List<SanPham> findByMau(long mauSac_id);

    @Query(value = "select * from san_pham\n" +
            "         join san_pham_chi_tiet on san_pham.id = san_pham_id\n" +
            "         where kich_co_id = ? and san_pham.is_deleted = false and trang_thai = 0", nativeQuery = true)
